...
function DownloadDLLs() {
  ...
  foreach ($DLL in $DLLs) {
    ...
    if ($quiet) {
      if ($DLL.FileName -eq "mdMatchup.dll" -or $DLL.FileName -eq "mdGlobalParse.dll")
      {
...
########################## Main ############################
Write-Host "`n======================= Melissa MatchUp Object Global =====================`n                          [ C++ | Windows | 64BIT ]`n"
...
if ([string]::IsNullOrEmpty($global) -and [string]::IsNullOrEmpty($us)){
  Push-Location MelissaMatchupObjectGlobalWindowsCpp
  & $BuildPath\MelissaMatchupObjectGlobalWindowsCpp.exe --license $License --dataPath $DataPath
  Pop-Location
}
else {
  Push-Location MelissaMatchupObjectGlobalWindowsCpp
  & $BuildPath\MelissaMatchupObjectGlobalWindowsCpp.exe --license $License --dataPath $DataPath --global $global --us $us 
  Pop-Location
}